...
interface CharacterAvatarProps {
  ...
  compact?: boolean; // when true, only render head (no name badge)
}

export function CharacterAvatar({ name, color, expression = "happy", compact = false }: CharacterAvatarProps) {
  const getEyes = () => {
    switch (expression) {
      case "happy":
        return (
  ...
  return (
    <motion.div className="flex flex-col items-center gap-2" whileHover={{ scale: 1.1, rotate: [0, -5, 5, -5, 0] }} transition={{ duration: 0.3 }}>
      <div className="relative">
        <svg width={compact ? "60" : "100"} height={compact ? "60" : "100"} viewBox="0 0 100 100" className="filter drop-shadow-[3px_3px_0px_#2d1b0e]">
          {/* Main head */}
